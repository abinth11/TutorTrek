...
const MenuBar: React.FC<{
...
}> = ({ commentId, message, updated, setUpdated }) => {
  ...
  return (
    <Menu placement='left-start'>
      ...
      <MenuList className='space-y-1 w-max' onClick={handleMenuClick}>
        <MenuItem className='flex items-center menu-item' data-action='Edit'>
          <span className='flex-grow'>Edit</span>
          <RiEdit2Line className='text-blue-500 text-sm flex-shrink-0' />
        </MenuItem>
        <MenuItem className='flex items-center menu-item' data-action='Delete'>
          <span className='flex-grow'>Delete</span>
          <RiDeleteBinLine className='text-red-500 text-sm flex-shrink-0' />
        </MenuItem>
        <MenuItem className='flex items-center menu-item' data-action='Report'>
...
const DiscussionListEl: React.FC<Props> = ({
  ...
  return (
    <li className='border-b mt-3 border-gray-300 p-6'>
      ...
      <h2>{studentId} kkk{studentDetails._id}</h2>
      <h2 className='mt-1'>{message}{studentId===studentDetails._id ?"edit":"no"}</h2>
      <div className='pt-1'>
